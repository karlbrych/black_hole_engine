...
bool Game::initialize() {
  ...
  window_ = glfwCreateWindow(WIDTH, HEIGHT, "black_hole_engine:OpenGL", nullptr,
                             nullptr);
  if (!window_) {
...
void Game::initializeScene() {
  ...
  lightSourceShader_ = std::make_unique<shader>(
      "shaders/vertex.glsl", "shaders/light_source_fragment.glsl");
  ...
  camera_.projection =
      glm::perspective(glm::radians(45.0f),
                       ...
                       10.0f, static_cast<float>(scale_) * 50.0f);
}
...
void Game::setupSolarSystem() {
  ...
  Object *sun = new Object(
      0, 0, 0, glm::vec3(0.0, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 1.0 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.25 * scale_), textures[2], false, true);

  Object *mercury = new Object(
      0, 0, std::sqrt(plane_.G / 0.387) * scale_,
      glm::vec3(0.387 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 1.65e-7 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.01 * scale_), textures[0], false);

  Object *venus = new Object(
      0, 0, std::sqrt(plane_.G / 0.723) * scale_,
      glm::vec3(0.723 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 2.45e-6 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.02 * scale_), textures[0], false);

  Object *earth = new Object(
      0, 0, std::sqrt(plane_.G / 1.0) * scale_,
      glm::vec3(1.0 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 3.00e-6 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.02 * scale_), textures[0], false);

  Object *mars = new Object(
      0, 0, std::sqrt(plane_.G / 1.524) * scale_,
      glm::vec3(1.524 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 3.23e-7 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.015 * scale_), textures[0], false);

  Object *jupiter = new Object(
      0, 0, std::sqrt(plane_.G / 5.203) * scale_,
      glm::vec3(5.203 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 9.54e-4 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.08 * scale_), textures[0], false);

  Object *saturn = new Object(
      0, 0, std::sqrt(plane_.G / 9.537) * scale_,
      glm::vec3(9.537 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 2.86e-4 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.07 * scale_), textures[0], false);

  Object *uranus = new Object(
      0, 0, std::sqrt(plane_.G / 19.191) * scale_,
      glm::vec3(19.191 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 4.37e-5 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.05 * scale_), textures[0], false);

  Object *neptune = new Object(
      0, 0, std::sqrt(plane_.G / 30.07) * scale_,
      glm::vec3(30.07 * scale_, 0.0, 0.0), sphereMesh_.VAO, sphereMesh_.VBO,
      sphereMesh_.EBO, 5.15e-5 * massScale, sphereMesh_.indexCount,
      static_cast<float>(0.05 * scale_), textures[0], false);
...
void Game::processInput() {
  ...
  if (!gameStopped_ && glfwGetKey(window_, GLFW_KEY_TAB) == GLFW_PRESS &&
      !tabPressedLastFrame_) {
    if (!objectEditor_.isOpen()) {
      ...
      objectEditor_.toggleOpen();
      firstMouse_ = true;
      glfwSetCursorPos(window_, static_cast<double>(editorMouseX_),
                       static_cast<double>(editorMouseY_));
    }
  }
  if (!gameStopped_) {
    glfwSetInputMode(window_, GLFW_CURSOR,
                     objectEditor_.isOpen() ? GLFW_CURSOR_NORMAL
...
void Game::renderRunningFrame() {
  ...
  plane_.draw(*objectShader_, *lightSourceShader_, camera_.projection,
              camera_.view, camPos_);
  if (!objectEditor_.isOpen()) {
    DoGravity(&plane_, plane_.G, plane_.dt);
  }

  if (objectEditor_.isOpen()) {
    objectEditor_.render(WIDTH + 10, HEIGHT + 10, &plane_, sphereMesh_,
                         textures[0], textures[1], textures[2], &camPos_,
                         &firstMouse_);
